...
export default function SegmentAnythingPage() {
  ...
  const [prompt, setPrompt] = useState<string>("a dog running in grass");
      ...
      {result.masks.length > 0 ? (
        <div className="grid sm:grid-cols-2 gap-4">
          {result.masks.map((mask, idx) => (
            <div key={`${idx}-${mask.score ?? "mask"}`} className="border rounded-lg p-2 bg-muted/20">
              <img
              ...
              <div className="mt-2 text-xs text-muted-foreground">
                <span>Score: {mask.score?.toFixed(3) ?? "n/a"}</span>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <TryLayout
        input={{
          title: "Input",
          description: "Upload an image and enter a text prompt.",
          children: (
            <div className="space-y-4">
              <ImageUpload
                label="Image"
